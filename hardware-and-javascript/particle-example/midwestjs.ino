...
int buildString(String arg) {
    String msgSant = sanitizeString(arg);
    Serial.println(msgSant);
    displayPlaceholder = msgSant;
    okayToSwitch = 1;
}
...
void setLED(uint32_t x, uint32_t y, uint32_t color){
  int ledAddr = (LEDsH * ((LEDsW-1)-x)) + (((x&1)==1)?((LEDsH-1)-y):(y));
